            ...
            if let userVariable = userVariable {
                if userVariable.value == nil {
                    if result is NSNumber {
                        userVariable.value = NSNumber(value: 0 as Int32) // FIXME: ensure correctness after Swift 4 conversion where value is used
                    } else {
                        userVariable.value = ""
                    }
                }
                if userVariable.value is NSNumber && result is NSNumber {
                    let number:NSNumber = (result as? NSNumber)! // FIXME!
