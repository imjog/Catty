...
@objc class CBSceneHelper: NSObject {
    ...
    @objc class func convertDegreesToScene(_ degrees: Double) -> Double {
        if degrees < 0.0 {
            return (-1 * (360.0 - PlayerConfig.RotationDegreeOffset) - (degrees.truncatingRemainder(dividingBy: -360.0))).truncatingRemainder(dividingBy: -360.0) // FIXME: ensure correctness after Swift 4 conversion and make more readable
        }
