...
class Firmata: FirmataProtocol {
    ...
    */
    func writePinMode(_ newMode:PinMode, pin:UInt8) {
        ...
        let newData:Data = Data(bytes: UnsafePointer<UInt8>(bytes), count: 3) // FIXME: ensure this and similar Swift 4 conversions were correct
...
//        let end2:UInt8 = 0
    ...
    */
    ...
    func i2cConfig(_ delay:UInt8,data:Data) {
        ...
        (data as NSData).getBytes(&bytes, length:count * MemoryLayout<UInt8>.size) // FIXME: ensure this and similar Swift 4 conversions were correct
    ...
    */
    private func parseAnalogMappingResponse(_ data:Data){
        ...
        for i in 2 ..< length {
            ...
            if(bytes[i] != 127){
                analogMapping.setObject(NSNumber(value: j as UInt8), forKey: NSNumber(value: bytes[i] as UInt8)) // FIXME: ensure this and similar Swift 4 conversions were correct
            }
